#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# 9x9 Quoridor-style tournament bot v2 (Python standard library only).
...
# Time budget per move: env QBOT_TIME (seconds, default 0.85).
...
try:
    TIME_BUDGET = float(os.environ.get("QBOT_TIME", "0.85"))
except Exception:
    TIME_BUDGET = 0.85

REP_W = 5.0           # penalty per recent visit to a square (anti-oscillation)
HIST = deque(maxlen=10)  # our recent pawn positions across turns
...
class Searcher(object):
    ...
    def pawn_moves(self, m):
        ...
        for d in dirs:
            n = p + d
            if ekey(p, n) in blocked:
                continue
            if n != q:
                res.append(n)
                continue
            qr, qc = divmod(q, N)
            ok = (d == -N and qr > 0) or (d == N and qr < 8) or (d == -1 and qc > 0) or (d == 1 and qc < 8)
            if ok and ekey(q, q + d) not in blocked:
                res.append(q + d)
            else:
                sides = (-1, 1) if d in (-N, N) else (-N, N)
                for e in sides:
                    ok2 = (e == -N and qr > 0) or (e == N and qr < 8) or (e == -1 and qc > 0) or (e == 1 and qc < 8)
                    if ok2:
                        s2 = q + e
                        if ekey(q, s2) not in blocked and s2 != p:
                            res.append(s2)
        return res
    ...
    def negamax(self, m, depth, alpha, beta, ply):
        ...
        if d_me == 0:
            return WIN - ply
        if depth <= 0:
            return (STEP_W * (d_op - d_me)
                    + WALL_W * (self.rem[m] - self.rem[1 - m])
                    + TEMPO_W)
...
def search_decide(state, budget):
    ...
    if not slots and pos[0] == parse_cell("e1") and pos[1] == parse_cell("e9"):
        HIST.clear()  # fresh game
    HIST.append(pos[m])
