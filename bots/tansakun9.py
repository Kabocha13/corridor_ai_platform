#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# 9x9 Quoridor-style tournament bot v3.0 (Python standard library only).
...
#   walls (the pawn race with body-blocking is then a solved game)
# - Effective-distance evaluation aware of body-blocking (no-jump ruleset)
...
def solve_pawn_race(adj):
    ...
    """
    nbrs = [neighbors_of(adj, u) for u in range(CELLS)]
    SZ = CELLS * CELLS * 2
    status = [0] * SZ
    dtm = [0] * SZ
    deg = [0] * SZ
    ...
    for a in range(CELLS):
        ...
        for b in range(CELLS):
            ...
            for m in (0, 1):
                ...
                wins = False
                succ = []
                for v in (na if m == 0 else nbrs[b]):
                    if v == y:
                        continue
                    if v // N == gr:
                        wins = True
...
class Searcher(object):
    ...
    def evaluate(self, m, d_me, d_op):
        ...
        pm = self.pos[m]
        po = self.pos[other]
        manh = abs(pm // N - po // N) + abs(pm % N - po % N)
        if d_me <= 6 and manh <= d_me + 2:
            db = blocked_dist(self.adj, self.pos[m], GOAL_ROW[m], self.pos[other])
            if db < 0:
                de_me = max(d_me, d_op + 2)
            elif db > d_me:
                de_me = min(db, d_me + 4)
        if d_op <= 6 and manh <= d_op + 2:
            db = blocked_dist(self.adj, self.pos[other], GOAL_ROW[other], self.pos[m])
            if db < 0:
...
def main():
    out = sys.stdout
    try:
        sys.stderr.write("tansakun5 v3.0 budget=%.2fs\n" % TIME_BUDGET)
        sys.stderr.flush()
    except Exception:
        pass
